 ...
 * Wipes and recreates the full category tree.
 */
...
async function seed() {
  ...
  // Delete products first (FK constraint), then categories
  await prisma.product.deleteMany({});
  await prisma.category.deleteMany({});
  console.log('Cleared existing data');

  // Insert roots
  const savedRoots = await Promise.all(
    ROOTS.map(r => prisma.category.create({ data: r }))
  );
  const bySlug = {};
  savedRoots.forEach(c => { bySlug[c.slug] = c.id; });
